...
def generate_pdf():
    ...
    try:
        ...
        if not pdf_path or not os.path.exists(pdf_path):
            ...
            print(f"❌ PDF generation failed for signal_id {signal_id}: {error_msg}")
        ...
        print(f"❌ Exception in generate_pdf endpoint: {e}")
        traceback.print_exc()
        return jsonify({"error": str(e)}), 500

# -----------------------------
# ✅ LEGACY ENDPOINT: For backward compatibility (symbol-based)
# -----------------------------
...
def download_pdf(symbol):
    ...
    try:
        ...
        if not pdf_path or not os.path.exists(pdf_path):
            print(f"❌ PDF not generated for symbol {symbol}")
            return jsonify({"error": "PDF not generated"}), 500

        print(f"✅ PDF successfully generated for symbol {symbol}: {pdf_path}")
        return send_file(
            pdf_path,
            mimetype="application/pdf",
            as_attachment=True,
        ...
        print(f"❌ Exception in download_pdf endpoint: {e}")
        traceback.print_exc()
        return jsonify({"error": str(e)}), 500
