...
def generate_live_signal_api(symbol: str) -> Dict[str, Any]:
    try:
        ...
        print(f"[SUCCESS] Signal generated: {final_signal} with confidence {confidence}")
        return result
...
def generate_pdf_report_full(symbol: str) -> Dict[str, Any]:
    ...
    try:
        ...
        print(f"[INFO] Plots saved. Backtest rows: {len(backtest_df)}, Price candles: {len(price_df)}")
...
def generate_pdf_for_signal(signal_id: str) -> Dict[str, Any]:
    ...
    try:
        ...
        plot_backtest_results(backtest_df, "reports/plots/backtest_chart.png")
        plot_price_with_indicators(price_df, backtest_df, symbol, "reports/plots/price_chart.png")
        signal_info['price_snapshot'] = f"{len(price_df)} candles"
        ...
        print(f"[INFO] PDF created at {pdf_path}, Backtest rows: {len(backtest_df)}, Price candles: {len(price_df)}")
