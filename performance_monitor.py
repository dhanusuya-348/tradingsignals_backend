...
from models import get_session_context, Signal, SignalPerformance, UserSignal
from algo.backtesting.live_tracker import monitor_live_signal
from notifications import send_performance_email
...
def process_signal_thread(signal_id, perf_id):
    """Thread to monitor a single signal in real-time and send performance emails"""
    try:
        ...
        if result:
            ...
            # FETCH PERFORMANCE DATA AND SEND EMAILS TO ALL USERS
            with get_session_context() as session:
                perf_obj = session.query(SignalPerformance).filter_by(id=perf_id).first()
                
                if perf_obj:
                    # Get all users watching this signal
                    user_signals = session.query(UserSignal).filter_by(signal_id=signal_id).all()
                    print(f"📧 Sending performance emails to {len(user_signals)} users")
                    
                    for us in user_signals:
                        try:
                            # Build performance email data from SignalPerformance columns
                            perf_data = {
                                "symbol": symbol,
                                "signal_id": signal_id,
                                "status": "COMPLETED",
                                "entry_price": perf_obj.exit_price,  # or entry_price if you have it
                                "exit_price": perf_obj.exit_price,
                                "exit_reason": perf_obj.exit_reason,
                                "pnl": perf_obj.profit_usd,
                                "pnl_percent": perf_obj.return_percent,
                                "trade_duration": f"{perf_obj.duration_minutes} minutes" if perf_obj.duration_minutes else "-",
                                "final_result": perf_obj.result,  # SUCCESS, FAILURE
                                "confidence": payload.get("confidence") if isinstance(payload, dict) else "N/A",
                                "sentiment": payload.get("sentiment") if isinstance(payload, dict) else "neutral",
                            ...
                            if success:
                                ...
                                print(f"   ❌ Failed to send to {us.email}")
                        
                        except Exception as e:
                            print(f"   [ERROR] Failed to email {us.email}: {e}")
                else:
                    print(f"⚠️ No performance record found for perf_id {perf_id}")
            
            return True
        else:
            print(f"❌ Signal {signal_id} failed or timed out")
                ...
                if signals:
                    ...
                    for sig in signals:
                        ...
                        if now >= valid_to:
                            ...
                            # ✅ Only update last_checked_id for EXPIRED signals
                            last_checked_id = max(last_checked_id, sig.id)
                        else:
